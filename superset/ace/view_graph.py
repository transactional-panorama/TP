...
class ViewGraph:
    ...
    def insert(self, dependency: Dependency) -> None:
        ...
        dep_list = self.prec_to_dep.get(prec, list())
        dep_list.append(dep)
        self.prec_to_dep[prec] = dep_list
    ...
    def create_snapshot_placeholder(self,
        ...
        for node_id in node_id_set:
            pending_queue.put(self.id_to_node[node_id])

        while not pending_queue.empty():
            cur_node = pending_queue.get()

            impacted_set.add(cur_node.node_id)
            result = {
                RESPONSE_CODE: 200,
                RESPONSE: "Done"
            }
            ...
            dep_list = self.prec_to_dep[cur_node.node_id]
            for dep_node in dep_list:
                if dep_node.node_id not in impacted_set:
                    pending_queue.put(dep_node)
