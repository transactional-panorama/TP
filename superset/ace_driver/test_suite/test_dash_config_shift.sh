...
REPORT_HOME="$TEST_HOME/experiment_results/dash_config/shift_step"
...
do
    for SHIFT_STEP in "${SHIFT_STEP_OPTIONS[@]}"
    do
    	for MVC_PROPERTY in "${MVC_PROPERTY_OPTIONS[@]}"
    	do
    	    STAT_DIR="$REPORT_HOME/SHIFT$SHIFT_STEP/MVC$MVC_PROPERTY/RUN$RUN"
    	    mkdir -p $STAT_DIR
    	    rm -f $STAT_DIR/*
