...
declare -a READ_BEHAVIOR_OPTIONS=("not_change" "regular_change" "see_change" "random_regular_change")
declare -a MVC_PROPERTY_OPTIONS=(1 2 3 4)

for RUN in `seq $START_RUN $END_RUN`
do
    VIEWPORT_START=$(( $RANDOM % ($CHART_NUM - $VIEWPORT_RANGE) ))
    ...
    do
	    ...
	    now="$(date)"
            echo "$now: Finished, VIEWPORT_START: $VIEWPORT_START; $REPORT_HOME/$READ_BEHAVIOR/MVC$MVC_PROPERTY/RUN$RUN" >> $TEST_HOME/test.log
    	done
    done
done 
